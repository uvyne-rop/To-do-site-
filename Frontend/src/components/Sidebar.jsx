...
import { setActiveView, addList } from '../store/tasksSlice';
...
export default function Sidebar({ onLogout, userEmail }) {
  ...
  const { tasks, lists, activeView } = useSelector(state => state.tasks);
  ...
  const getTaskCount = view => {
    switch (view) {
      ...
      case 'planned':
        return tasks.filter(t => t.date && !t.completed).length;
      case 'all':
        return tasks.filter(t => !t.completed).length;
      default:
        return tasks.filter(t => t.list === view && !t.completed).length;
    }
  };
          ...
          <div className="space-y-1">
            {lists.map(list => {
              ...
              return (
                <button
                  key={list.id}
                  onClick={() => dispatch(setActiveView(list.id))}
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-all ${
                    isActive
                      ? 'bg-blue-50 text-blue-700'
                      : 'text-gray-700 hover:bg-gray-50'
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-2 h-2 rounded-full ${colorMap[list.color]}`}
                    />
                    <span
                      className={`font-medium ${
                        isActive ? 'text-blue-700' : ''
